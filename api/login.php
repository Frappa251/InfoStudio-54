...
// Accettiamo solo richieste POST: il login NON va mai fatto in GET
// (la password finirebbe nell'URL e nei log del server).
...
if ($utente == false || !password_verify($password, $utente['password_hash'])) {
    // ATTENZIONE: diamo lo stesso messaggio sia che l'email non esista
...
// Salviamo l'id dell'utente in sessione. Da questo momento, finché la
// sessione è viva, current_user() troverà i suoi dati.
$_SESSION['user_id'] = (int) $utente['id'];

// Rispondiamo con i dati dell'utente (senza la password!) così il
// frontend può mostrare il nome nella navbar, ecc.
json_response([
    'success' => true,
